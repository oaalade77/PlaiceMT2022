...
run.dir <- "RT.2021.Final.Model"
dat.filename <- "PlaiceWHAM-2019_revised_NEFSC-LW-WAA_splitNEFSC-BigUnit.DAT"
...
WHAM_Run29F4_init <- fit_wham(input, MakeADFun.silent = TRUE, do.osa = FALSE, do.retro=FALSE) 
check_convergence(WHAM_Run29F4_init)
print(paste("Number of parameters", length(WHAM_Run29F4_init$par), sep=" "))

# ID ages that should be fixed at full selectivity
#divide by max selectivity
t(sapply(WHAM_Run29F4_init$rep$selAA[2:(nindices+1)], function(x) x[1,]/max(x[1,])))
#WHAM_Run29F4_init$rep$selAA[2][[1]][1,] # Albatross spring, max sel = age 6
#WHAM_Run29F4_init$rep$selAA[3][[1]][1,] # Bigelow spring, max sel = age 5
#WHAM_Run29F4_init$rep$selAA[4][[1]][1,] # Albatross fall, max sel = age 4
#WHAM_Run29F4_init$rep$selAA[5][[1]][1,] # Bigelow fall, max sel = age 3
```
...
# The following code can be used to fix selectivity for a given age
...
saveRDS(input, file=file.path(run.dir, "WHAM_Run29F4_input.rds"))
```
...
if(run.osa == FALSE) 
{
  # Run without OSA to check fit and retro 
  ...
  plot_wham_output(mod=WHAM_Run29F4_basic, out.type='html')
  plot_wham_output(mod=WHAM_Run29F4_basic, out.type='png', dir.main = basic_dir)
  
  saveRDS(WHAM_Run29F4_basic, file= file.path(run.dir, "WHAM_Run29F4_basic_model.rds"))
}
...
if(run.osa == TRUE) 
{
  ...
  WHAM_Run29F4_osa <- fit_wham(input, MakeADFun.silent = TRUE, do.osa = TRUE) 
  check_convergence(WHAM_Run29F4_osa)
  print(paste("Number of parameters", length(WHAM_Run29F4_osa$par), sep=" "))
  
  osa_dir <- file.path(run.dir, 'osa_results')
  if(!dir.exists(osa_dir)) {dir.create(osa_dir)}
  
  plot_wham_output(mod=WHAM_Run29F4_osa, out.type='html')
  plot_wham_output(mod=WHAM_Run29F4_osa, out.type='png', dir.main = osa_dir)

  saveRDS(WHAM_Run29F4_osa, file= file.path(run.dir, "WHAM_Run29F4_osa_model.rds"))
}
...
data.frame(age = 1:nage, albSpring = albSpring, bigSpring = bigSpring, albFall = albFall, bigFall = bigFall) %>%
  ggplot() +
