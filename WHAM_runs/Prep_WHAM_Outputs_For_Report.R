...
proj40 <- readRDS(file.path(run.dir, f40.rds))
...
# Reference points
Fproxy <- exp(WHAM_basic$rep$log_FXSPR_static)
  names(Fproxy) <- 'est'

SSBproxy.vec <- data.frame(log.est = WHAM_basic$rep$log_SSB_FXSPR_static,
                           log.se  = as.list(WHAM_basic$sdrep, what = "Std", report = TRUE)$log_SSB_FXSPR_static) %>%
  mutate( est = exp(log.est),
          lo = exp(log.est - qnorm(0.975)*log.se),
          hi = exp(log.est + qnorm(0.975)*log.se)
  )

MSYproxy.vec <- data.frame(log.est = WHAM_basic$rep$log_Y_FXSPR_static,
                           log.se  = as.list(WHAM_basic$sdrep, what = "Std", report = TRUE)$log_Y_FXSPR_static) %>%
  mutate( est = exp(log.est),
          lo = exp(log.est - qnorm(0.975)*log.se),
          hi = exp(log.est + qnorm(0.975)*log.se)
  )
...
relF <- F.yr %>%
  ...
  select(Year, est, relF)

relSSB <- SSB.yr %>%
  mutate(relSSB = est/SSBproxy) %>%
  select(Year, est, relSSB)
