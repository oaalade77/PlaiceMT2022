---
title: "Run 3: Updated parameterization of FXSPR_static"
...
## Model description
Fix specification of FXSPR_static to be based on the last five years 
...
dat.filename <- "2022_AMP_MT_ASAP_WHAM_MODEL_INPUT_REVISED_WAA.DAT"
...
# 1) ASAP-like
NAA_re = list(sigma = "rec") # Recruitment as random effects, but ASAP-like; To compare to rec+1 in terms of AIC
NAA_re$cor = "iid"             # iid random effects
NAA_re$recruit_model = 2       # recruitment is random about the mean
NAA_re$recruit_pars = exp(10)  #initial guess for mean recruitment

# 2) Fixed-effect recruitment
### Do not specify NAA_re at all
...
### To turn off random effects in selectivity
sel_list <- list(model = modelsetup, # list selectivity model for each fleet and index
                 initial_pars = c(init_fleet_sel, init_index_sel),
                 fix_pars = c(fix_fleet_sel, fix_index_sel))
...
input <- prepare_wham_input(asap3, 
                            NAA_re = NAA_re, #### TAKE OUT WHEN NOT RUNNING ANY RANDOM EFFECTS
                            selectivity = sel_list, 
                            model_name = run.name, 
                            age_comp = "logistic-normal-miss0" # logistic normal age comp, 0s treated as missing
                           )
...
WHAM_basic <- fit_wham(input, MakeADFun.silent = TRUE, do.osa = FALSE, do.retro=TRUE) 
