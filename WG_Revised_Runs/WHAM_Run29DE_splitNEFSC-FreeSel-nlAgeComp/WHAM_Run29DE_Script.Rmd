...
randeffect <- c(rep("none", asap3$dat$n_fleet_sel_blocks),  "none", rep("none", 3)) # Don't include 
...
WHAM_Run29a_free <- fit_wham(input, do.osa=F, do.retro=F) 
check_convergence(WHAM_Run29a_free)
q_sel = t(sapply(1:4, function(x) {
  sel = WHAM_Run29a_free$rep$QAA[1,x,]
  q = max(sel)
  sel = sel/q
  return(c(q,sel))
}))
...
#fix_index_sel[[1]] <- 4 # Fix age 4 & 5 for for index 1 (NEFSC spring Albatross)
#init_index_sel[[1]][4] <- 1
fix_index_sel[[1]] <- c(4) # Fix age 4 & 5 for for index 1 (NEFSC spring Albatross)
fix_index_sel[[2]] <- 4:5 # Fix age 4 & 5 for for index 2 (NEFSC spring Bigelow)
fix_index_sel[[3]] <- c(4:5,11) # Fix age 4 for for index 2 (NEFSC fall Albatross)
fix_index_sel[[4]] <- 4 # Fix age 4 & 5 for for index 2 (NEFSC spring Bigelow)
init_index_sel <- lapply(1:use_n_indices, function(x) c(0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5))
for(i in 1:4) init_index_sel[[i]][fix_index_sel[[i]]] <- 1

randeffect <- c(rep("iid", asap3$dat$n_fleet_sel_blocks),  "iid", rep("none", 3)) # Don't include 
sel_list <- list(model = modelsetup, # list selectivity model for each fleet and index
                 re = randeffect,
                 initial_pars = c(init_fleet_sel, init_index_sel),
                 fix_pars = c(fix_fleet_sel, fix_index_sel))
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, 
  model_name = "WHAM_Run29a_ln_acomp", age_comp = "logistic-normal-miss0")
WHAM_Run29a_ln_acomp <- fit_wham(input, do.osa=F, do.retro = F, MakeADFun.silent = TRUE) 

q_sel = t(sapply(1:4, function(x) {
  sel = WHAM_Run29a_ln_acomp$rep$QAA[1,x,]
  q = max(sel)
  sel = sel/q
  return(c(q,sel))
}))
...
# selectivity random effects for surveys, use to freely estimate selectivity for fleets
...
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run29_splitNEFSC", "WHAM_Run29a_ln_acomp_input.rds", sep="/"))
```
...
input$par = WHAM_Run29a_ln_acomp$parList
WHAM_Run29a_ln_acomp <- fit_wham(input, MakeADFun.silent = TRUE) 
check_convergence(WHAM_Run29a_ln_acomp)
print(paste("Number of parameters", length(WHAM_Run29$par), sep=" "))

plot_wham_output(mod=WHAM_Run29a_ln_acomp, out.type='html')
...
Fleet OSA residuals are less normally distributed than in Run27. OSA residuals for indices 1, 2, and 4 (Albatross spring, Bigelow spring, Bigelow fall) were fairly normally distributed, with idnex 3 (Albatross fall) less normally distributed
