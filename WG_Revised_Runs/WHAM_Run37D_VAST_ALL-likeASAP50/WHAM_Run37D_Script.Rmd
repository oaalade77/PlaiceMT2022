---
title: "WHAM_Run37D_VAST_ALL-likeASAP50"
...
VAST data was taken from ASAP run 50 (order of split indices different than run 37C, here: Albatross spring/fall, Bigelow spring/fall)
...
### Load data
```{r}
asap3 <- read_asap3_dat(paste(here::here(), "data", "PlaiceWHAM-2019_revised_NEFSC-LW-WAA_StateVAST-asASAP50.DAT", sep="/"))
...
init_index_sel <- list(c(0.5, 0.5, 1, 1, 1, 0.5, 0.5, 0.5, 0.5, 0, 0), # all VAST Albatross spring
                       c(0.5, 1, 1, 1, 1, 0.5, 0.5, 0.5, 0.5, 0, 0), # all VAST Albatross fall
                       c(0.5, 0.5, 0.5, 1, 1, 0.5, 0.5, 0.5, 0.5, 0, 0), # all VAST Bigelow spring
                       c(0.5, 0.5, 1, 1, 1, 0.5, 0.5, 0.5, 0.5, 0, 0)) # all VAST Bigelow fall

# Setup fixed parameters
fix_fleet_sel <- lapply(1:asap3$dat$n_fleets, function(x) NA) 
fix_index_sel <- lapply(1:use_n_indices, function(x) NA) # Setup storage object to populate 

fix_index_sel[[1]] <- c(3,4,5) # Fix age 3 & 4 for for index 1 all VAST Albatross spring
fix_index_sel[[2]] <- c(2,3,4,5) # Fix age 5 for for index 2 all VAST Albatross fall
fix_index_sel[[3]] <- c(4,5) # VAST Bigelow spring
fix_index_sel[[4]] <- c(3,4,5) # VAST Bigelow fall
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, model_name = "WHAM_Run37D") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run37D_VAST_ALL-likeASAP50", "WHAM_Run37D_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run37D <- fit_wham(input, MakeADFun.silent = TRUE, do.osa=FALSE) 
check_convergence(WHAM_Run37D)
print(paste("Number of parameters", length(WHAM_Run37D$par), sep=" "))

plot_wham_output(mod=WHAM_Run37D, out.type='html')
# plot_wham_output(mod=WHAM_Run37D, out.type='png', dir.main = paste(here::here(), "WHAM_Run37D", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run37D_VAST_ALL-likeASAP50/WHAM_Run37D_input.rds", sep="/"))

# Rerun data
ReRun37D <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
Model did not converge with invertible hessian using same selectivity assumptions as run 37C, tried freely estimating selectivity and using this to inform ages fixed at full selectivity. 
...
modelRuns <- paste(here::here(), "WG_Revised_Runs",
                   c("WHAM_Run37B_VAST_ALL-split/WHAM_Run37B_model.rds",
                     "WHAM_Run37D_VAST_ALL-likeASAP50/WHAM_Run37D_model.rds"), sep="/")
