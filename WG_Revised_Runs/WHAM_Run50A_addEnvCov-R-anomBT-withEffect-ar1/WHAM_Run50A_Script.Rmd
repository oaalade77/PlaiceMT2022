---
title: "WHAM_Run50A_addEnvCov-R-anomBT-withEffect-ar1"
output: html_document
date: '2022-05-17'
---

## Model description
Model specified as in run 50A (fit with an environmental covariate that has no effect so it can be directly compared to run 50A that incorporates this environmental effect on catchability) but implements an ar1 process rather than a random walk. Annual bottom temperature anomaly was the environmental covariate considered here.
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run50A") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run50A_addEnvCov-R-anomBT-withEffect-ar1", "WHAM_Run50A_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run50A <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run50A)
print(paste("Number of parameters", length(WHAM_Run50A$par), sep=" "))

plot_wham_output(mod=WHAM_Run50A, out.type='html')
# plot_wham_output(mod=WHAM_Run50A, out.type='png', dir.main = paste(here::here(), "WHAM_Run50A", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run50A, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run50A_addEnvCov-R-anomBT-withEffect-ar1", "WHAM_Run50A_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run50A_addEnvCov-R-anomBT-withEffect-ar1/WHAM_Run50A_input.rds", sep="/"))

# Rerun data
ReRun50A <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
Fit to the BT anomaly data was similar in run 50 and 50A. 

Age comp OSA residuals for the fleet and indices had similar patterns and distributions in both runs 50 and 50A. OSA residuals for fit to aggregate data was also similar across these runs. 

OSA residuals for fit to BT anomaly data were not very normally distributed in either run.

SSB and R estimates are slightly lower and F slighlty higher at the end of the timeseries in run 50A compared to run 50, and CVs around estimates are slightly higher for SSB, R, and F at the end of the time series. 

AIC was smaller for run 50A but was within +/-2 of the AIC value for run 50 so these runs should be considered equivalent. Mohn's rho values were smaller in run 50A for SSB and F but larger for R. Run 50 is the preferred run because the AIC scores are equivalent, Mohn's rho values are generally lower and run 50 fits to less data.
