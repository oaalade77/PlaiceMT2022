...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, 
  catchability = catchability, model_name = "WHAM_Run29")
...
fix_index_sel[[3]] <- 4 # Fix age 4 for for index 2 (NEFSC fall Albatross)
...
sel_list <- list(model = modelsetup, # list selectivity model for each fleet and index
                 ...
                 fix_pars = c(fix_fleet_sel, fix_index_sel))
catchability <- list(
  initial_q = c(0.3,2,0.3,2))
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, 
  catchability = catchability, model_name = "WHAM_Run29")
input$map$logit_q = factor(c(1,NA,2,NA)) #don't estimate q
WHAM_Run29a_free <- fit_wham(input, do.osa=F, do.retro=F) 
