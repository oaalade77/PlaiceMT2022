---
title: "WHAM_Run37_VAST_ALL"
output: html_document
date: '2022-04-04'
---

## Model description
A full state-space model was implemented as in run 27 (logistic fleet selectivity, age-specific index selectivity and recruitment about the mean), but fit to spring and fall VAST model-based indices that integrates over all design-based indices except LPUE rather than fitting to design-based NEFSC, MADMF, MENH spring and fall indices. and no iid random effect for selectivity, 
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, model_name = "WHAM_Run37") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run37_VAST_ALL", "WHAM_Run37_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run37 <- fit_wham(input, MakeADFun.silent = TRUE, do.osa=FALSE) 
check_convergence(WHAM_Run37)
print(paste("Number of parameters", length(WHAM_Run37$par), sep=" "))

plot_wham_output(mod=WHAM_Run37, out.type='html')
# plot_wham_output(mod=WHAM_Run37, out.type='png', dir.main = paste(here::here(), "WHAM_Run37", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run37, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run37_VAST_ALL", "WHAM_Run37_model.rds", sep="/"))
```
...
WHAM_Run37$parList
WHAM_Run37$rep$logit_selpars

# Look at parameter estimates
WHAM_Run37$sdrep

# debug NA/NaN
WHAM_Run37A <- fit_wham(input, do.fit=F)
therep = WHAM_Run37A$report()
names(therep)
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run37_VAST_ALL/WHAM_Run37_input.rds", sep="/"))

# Rerun data
ReRun37 <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment


Poor fit to index 1 (all VAST spring).
