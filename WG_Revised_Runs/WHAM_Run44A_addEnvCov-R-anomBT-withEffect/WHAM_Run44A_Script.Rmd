---
title: "WHAM_Run44A_addEnvCov-R-anomBT-withEffect"
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run44A") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run44A_addEnvCov-R-anomBT-withEffect", "WHAM_Run44A_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run44A <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run44A)
print(paste("Number of parameters", length(WHAM_Run44A$par), sep=" "))

plot_wham_output(mod=WHAM_Run44A, out.type='html')
# plot_wham_output(mod=WHAM_Run44A, out.type='png', dir.main = paste(here::here(), "WHAM_Run44A", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run44A, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run44A_addEnvCov-R-anomBT-withEffect", "WHAM_Run44A_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run44A_addEnvCov-R-anomBT-withEffect/WHAM_Run44A_input.rds", sep="/"))

# Rerun data
ReRun44A <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
Estimates of SSB and recruitment are slightly lower for run 44A than run 44. CVs around SSB, R, and F estimates are slightly higher at the end of the time series for run 44A. 

OSA residuals for fit to aggregate fleet and indices were similarly distributed in runs 44 and 44A. OSA residuals for fit to age comp data were similar in magnitude and pattern in both run 44 and 44A. The BT anomaly OSA residuals were not calculated/plotted for this run. 

AIC was larger in run 44A but still within 2 of the AIC score for run 44 so this run should be considered equivalent. Mohn's rho values were larger than for run 44. Slight preference for run 44 since no major improvements in 44A. 

Note: not all diagnostic plots were automatically generated for this run.
