---
title: "WHAM_Run42_addEnvCov-R-NAO-noEffect"
output: html_document
date: '2022-05-16'
---

## Model description
Model specified as in run 29B with 2 major differences, 1) both age 4 and 5 were fixed at full selectivity for all indices (impacts Bigelow spring and Albatross fall) and 2) this run is fit with an environmental covariate that has no effect so it can be directly compared to run 42A that incorporates this environmental effect on recruitment. The North Atlantic Oscillation (NAO) Annual was the environmental covariate considered here, with a 1 year lag (bottom temperature in year t impacts recruitment in year t+1) and a random walk process model.
...
The run did not converge when specified as in run 29B when fit to the environment covariate so a run was conducted to freely estimate selectivity to update the settings for the full run42
```{r}
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run42") 

WHAM_Run42 <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run42)
print(paste("Number of parameters", length(WHAM_Run42$par), sep=" "))
plot_wham_output(mod=WHAM_Run42, out.type='html')
```
...
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run42_addEnvCov-R-NAO-noEffect", "WHAM_Run42_input.rds", sep="/"))
```
...
plot_wham_output(mod=WHAM_Run42, out.type='html')
# plot_wham_output(mod=WHAM_Run42, out.type='png', dir.main = paste(here::here(), "WHAM_Run42", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run42, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run42_addEnvCov-R-NAO-noEffect", "WHAM_Run42_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run42_addEnvCov-R-NAO-noEffect/WHAM_Run42_input.rds", sep="/"))

# Rerun data
ReRun42 <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
There were no major differences with run 42A (see comments in that run script), except for worse fit to NAO time series. AIC was larger than run 42A, but most Mohn's rho values for run 42 were smaller. The improvement in the AIC score for run 42A could be attributed to the improved fit to the NAO time series, but did not improve fit to any of the other indices so run 42 still appears to be a more appropriate model.
