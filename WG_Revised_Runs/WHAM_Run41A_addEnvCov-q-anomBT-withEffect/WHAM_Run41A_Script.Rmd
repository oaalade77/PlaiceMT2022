...
!!! THIS RUN SHOULD US BT OR A BT ANOMALY RATHER THAN SST
...
# anomSST <- rbind(c(1979, -999, -999, -999, -999), anomSST) # no lag
```
...
# Setup environmental covariate and options
  ecov <- list(
    ...
    process_model = "rw",
    where = "q", # effect on catchability
    how = 1, # link turned on
...
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run41A_addEnvCov-anomSST-withEffect", "WHAM_Run41A_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run41A <- fit_wham(input, do.osa = FALSE, do.retro = TRUE) 
...
```{r}
# Save fitted model
saveRDS(WHAM_Run41A, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run41A_addEnvCov-anomSST-withEffect", "WHAM_Run41A_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run41A_addEnvCov-anomSST-withEffect/WHAM_Run41A_input.rds", sep="/"))
...
## Comment
Index 3 is not a perfectly straight line, but doesn't look well estimated given that there is basically no change over time for this index but there is variability in the other indices
