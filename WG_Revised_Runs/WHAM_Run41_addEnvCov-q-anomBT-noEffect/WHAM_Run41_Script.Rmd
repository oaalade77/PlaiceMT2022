---
title: "WHAM_Run41_addEnvCov-q-anomBT-noEffect"
output: html_document
date: '2022-05-16'
---

## Model description
Model specified as in run 29B but is fit with an environmental covariate that has no effect so it can be directly compared to run 41A and 41B that incorporates this environmental effect on catchability. Annual bottom temperature anomaly was the environmental covariate considered here.
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run41") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run41_addEnvCov-q-anomBT-noEffect", "WHAM_Run41_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run41 <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run41)
print(paste("Number of parameters", length(WHAM_Run41$par), sep=" "))

plot_wham_output(mod=WHAM_Run41, out.type='html')
# plot_wham_output(mod=WHAM_Run41, out.type='png', dir.main = paste(here::here(), "WHAM_Run41", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run41, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run41_addEnvCov-q-anomBT-noEffect", "WHAM_Run41_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run41_addEnvCov-q-anomBT-noEffect/WHAM_Run41_input.rds", sep="/"))

# Rerun data
ReRun41 <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
Minor differences with run 41A (see comments in that run script), particularly due to the catchability random effect. AIC was slightly larger than run 41A, but Mohn's rho values were slightly smaller and there no other obvious improvements in OSA residuals, suggesting that run 41 should still be the preferable run.
