# source("/home/bstock/Documents/ms/butterfish/code/best_v4.R")
...
res_dir <- here("results","best_v4")
dir.create(res_dir, showWarnings=FALSE)

# mods <- readRDS("/home/bstock/Documents/ms/butterfish/results/best_v3/mods.rds")
# df <- readRDS("/home/bstock/Documents/ms/butterfish/results/best_v3/mase.rds")
# mods <- mods[1:3]
# mods[[4]] <- readRDS("/home/bstock/Documents/ms/butterfish/results/25_1selblock/NAA2-ln-agesp-2dar1_noproj.rds")
# names(mods) <- c("04-Base","04-NAA2","17-NAA4","25-NAA2")
mods <- readRDS("/home/bstock/Documents/ms/butterfish/results/best_v4/mods.rds")
df <- readRDS("/home/bstock/Documents/ms/butterfish/results/best_v4/mase.rds")
mods[[5]] <- readRDS("/home/bstock/Documents/ms/butterfish/results/25_1selblock/Base-ln-2dar1_noproj.rds")
mods[[6]] <- readRDS("/home/bstock/Documents/ms/butterfish/results/25_1selblock/NAA2-ln-2dar1_noproj.rds")
mods[[7]] <- readRDS("/home/bstock/Documents/ms/butterfish/results/25_1selblock/NAA4-ln-2dar1_noproj.rds")
names(mods) <- c("04-Base","04-NAA2","17-NAA4","25-NAA2-selAA","25-Base-FAA","25-NAA2-FAA","25-NAA4-FAA")
df$model[df$model == "25-NAA2"] = "25-NAA2-selAA"
res <- compare_wham_models(mods, fdir=res_dir, do.table=T, table.opts=list(sort=F, calc.rho=F), plot.opts=list(which=c(1,2,5,8:9))) # don't do kobe plot bc fxspr isn't estimable
...
df$model = factor(df$model, levels=c("04-Base","04-NAA2","17-NAA4","25-NAA2-selAA","25-Base-FAA","25-NAA2-FAA","25-NAA4-FAA"), labels=c("04-Base","04-NAA2","17-NAA4","25-NAA2-selAA","25-Base-FAA","25-NAA2-FAA","25-NAA4-FAA"))
...
# 4 25-NAA2-selAA 0.73 0.79 0.84 0.74 0.45
# 5   25-Base-FAA 0.75 0.91 0.72 0.95 0.68
# 6   25-NAA2-FAA 0.77 0.88 0.76 0.99 0.71
# 7   25-NAA4-FAA 0.74 0.86 0.72 0.97 0.68
...
# 4 25-NAA2-selAA 0.73 0.88 0.79 1.11 0.56
# 5   25-Base-FAA 0.79 0.90 0.91 1.05 0.70
# 6   25-NAA2-FAA 0.78 0.91 0.92 1.15 0.69
# 7   25-NAA4-FAA 0.73 0.87 0.88 1.06 0.64
...
# --------------------------------------------------------
# Plot selAA and NAA patterns (4 panel)
mod.list <- c(here("results","25_1selblock","NAA2-ln-agesp-2dar1.rds"),
              here("results","25_1selblock","Base-ln-2dar1.rds"),
              here("results","25_1selblock","NAA2-ln-2dar1.rds"),
              here("results","25_1selblock","NAA4-ln-2dar1.rds"))
mods <- lapply(mod.list, readRDS)
names(mods) <- c("25-NAA2-selAA","25-Base-FAA","25-NAA2-FAA","25-NAA4-FAA")
n.mods <- length(mods)
source(here("code","plot_FAA.R"))
g <- plot_selAA_facet(mods)
png(file.path(res_dir, "selAA_panel.png"), units='in', res=200, width=7, height=7)
print(g)
dev.off()

source(here("code","plot_naa_devs.R"))
g <- plot_naa_devs(mods=mods, cor = c('ar1_y','none','ar1_y','2dar1'), sig = c('rec','none','rec','rec+1'))
png(file.path(res_dir,"naa_devs.png"), height=7, width=7, units='in',res=200)
print(g)
dev.off()

mod.list <- c(here("results","04_RUN_036_NAA_fix1q0.1975_fix1sel","Base.rds"),
  here("results","04_RUN_036_NAA_fix1q0.1975_fix1sel","NAA-2.rds"),
  here("results","17_ln_fix1q0.1975","NAA-4.rds"))
mods <- lapply(mod.list, readRDS)
names(mods) <- c("04-Base","04-NAA2","17-NAA4")
n.mods <- length(mods)
g <- plot_selAA_facet(mods)
png(file.path(res_dir, "selAA_panel_old.png"), units='in', res=200, width=7, height=5.5)
print(g)
dev.off()

g <- plot_naa_devs(mods=mods, cor = c('none','ar1_y','2dar1'), sig = c('none','rec','rec+1'))
png(file.path(res_dir,"naa_devs_old.png"), height=5.5, width=7, units='in',res=200)
print(g)
dev.off()
...
df <- readRDS(file.path(res_dir, "mase.rds"))
mase <- filter(df, model %in% c("04-Base","04-NAA2","17-NAA4"))
saveRDS(mase, file="/home/bstock/Documents/ms/butterfish-wham/results/mase.rds")


dplyr::bind_rows(allpars, .id = "model")
