---
title: "WHAM_Run45A_addEnvCov-q-AMO-withEffect"
output: html_document
date: '2022-05-10'
---

## Model description
Model specified as in run 29B with 2 major differences, 1) only age 5 in spring indices and age 4 in fall indices were fixed at full selectivity and 2) this run was fit with an environmental covariate with an effect on catchability. The Atlantic Multidecadal Oscillation (AMO) was the environmental covariate considered here.
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run45A") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run45A_addEnvCov-q-AMO-withEffect", "WHAM_Run45A_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run45A <- fit_wham(input, do.osa = FALSE, do.retro = FALSE) 
check_convergence(WHAM_Run45A)
print(paste("Number of parameters", length(WHAM_Run45A$par), sep=" "))

plot_wham_output(mod=WHAM_Run45A, out.type='html')
# plot_wham_output(mod=WHAM_Run45A, out.type='png', dir.main = paste(here::here(), "WHAM_Run45A", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run45A, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run45A_addEnvCov-q-AMO-withEffect", "WHAM_Run45A_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run45A_addEnvCov-q-AMO-withEffect/WHAM_Run45A_input.rds", sep="/"))

# Rerun data
ReRun45A <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
Base run 45 did not converge, nor did this run which includes the effect of AMO on catchability. Catchability estimates had very high confidence bounds and did not appear to be well estimated which could be one reason the model did not converge.
