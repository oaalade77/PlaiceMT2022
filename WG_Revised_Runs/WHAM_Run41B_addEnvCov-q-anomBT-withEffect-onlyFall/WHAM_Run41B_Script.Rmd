...
## Model description
Model specified as in run 41 but incorporates the environmental effect on catchability for only the fall Albatross and Bigelow indices. Annual sea surface temperature anomaly was the environmental covariate considered here.
...
# Setup environmental covariate and options
  ecov <- list(
    ...
    indices = list(3) # Just Fall albatross
...
```{r}
temp <- fit_wham(input, do.osa = FALSE, do.retro = FALSE, MakeADFun.silent = TRUE) 
check_convergence(temp)
WHAM_Run41B <- fit_wham(input, do.osa = FALSE, do.retro = TRUE) 
