...
THE GOAL FOR THIS RUN IS TO FREELY ESTIMATE SELECTIVITY AT OLDER AGES BUT FORCE THE 11+ GROUP FOR ALBATROSS FALL TO MATCH THAT OF A YOUNGER AGE. 
...
WHAM_Run29F5 <- fit_wham(input, MakeADFun.silent = TRUE, do.osa = FALSE, do.retro=FALSE) 
...
t(sapply(WHAM_Run29F5$rep$selAA[2:5], function(x) x[1,]/max(x[1,])))
#WHAM_Run29F5$rep$selAA[2][[1]][1,] # Albatross spring, max sel = age 6
#WHAM_Run29F5$rep$selAA[3][[1]][1,] # Bigelow spring, max sel = age 5
#WHAM_Run29F5$rep$selAA[4][[1]][1,] # Albatross fall, max sel = age 4
#WHAM_Run29F5$rep$selAA[5][[1]][1,] # Bigelow fall, max sel = age 3
```
...
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run29F-5_splitNEFSC-BigUnits-nlAgeComp-11Match", "WHAM_Run29F5_input.rds", sep="/"))
```
...
randeffect <- c(rep("iid", asap3$dat$n_fleet_sel_blocks), c("none","none","ar1","none")) # Don't include selectivity random effects for any surveys 
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, model_name = "WHAM_Run29F5", age_comp = "logistic-normal-miss0") # logistic normal age comp, 0s treated as missing
# TJM: I believe I have uncovered a "bug" in the configuration of "ar1" with age-specific selectivity. When these are chosen, any logit_selpars that are estimated should be set to be a single "mean" value for the ar1 process. Doing this by hand below.
temp = input$par$logit_selpars
temp[] = as.integer(input$map$logit_selpars)
...
input$par = WHAM_Run29F5_basic$parList #start at the optimized values (from run without OSA, retro) to save time
...
```{r}
# Save fitted model
saveRDS(WHAM_Run29F5, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run29F-5_splitNEFSC-BigUnits-nlAgeComp-11Match", "WHAM_Run29F5_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run29F-5_splitNEFSC-BigUnits-nlAgeComp-11Match/WHAM_Run29F5_input.rds", sep="/"))
...
modelRuns <- paste(here::here(), "WG_Revised_Runs",
                   c("WHAM_Run29B_splitNEFSC-BigUnits-noSurvRandSel/WHAM_Run29B_model.rds",
                     "WHAM_Run29F-5_splitNEFSC-BigUnits-nlAgeComp-11Match/WHAM_Run29F5_model.rds"), sep="/")
# Read in model Rdata
...
albSpringSel <- models$Run29F5$rep$selAA[[2]][1,] # Albatross spring
bigSpringSel <- models$Run29F5$rep$selAA[[3]][1,] # Bigelow spring
albFallSel <- models$Run29F5$rep$selAA[[4]][1,] # Albatross fall
bigFallSel <- models$Run29F5$rep$selAA[[5]][1,] # Bigelow fall
