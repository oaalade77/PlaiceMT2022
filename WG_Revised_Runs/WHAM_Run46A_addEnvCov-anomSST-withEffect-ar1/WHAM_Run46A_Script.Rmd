---
title: "WHAM_Run46A_addEnvCov-anomSST-withEffect-ar1"
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run46A") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run46A_addEnvCov-anomSST-withEffect-ar1", "WHAM_Run46A_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run46A <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run46A)
print(paste("Number of parameters", length(WHAM_Run46A$par), sep=" "))

plot_wham_output(mod=WHAM_Run46A, out.type='html')
# plot_wham_output(mod=WHAM_Run46A, out.type='png', dir.main = paste(here::here(), "WHAM_Run46A", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run46A, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run46A_addEnvCov-anomSST-withEffect-ar1", "WHAM_Run46A_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run46A_addEnvCov-anomSST-withEffect-ar1/WHAM_Run46A_input.rds", sep="/"))

# Rerun data
ReRun46A <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
OSA residuals for fit to aggregate fleet and albatross indices were similarly distributed, while Bigelow residuals for both spring and fall were slightly less normally distributed than in run 46. OSA residuals for fit to the age comp data were similar in both run 46 and 46A. 

OSA residuals for fit to the SST anomaly time series was not normally distributed for run 46A and were not plotted in run 46. 

R and SSB estimates were slighlty higher and F estimates slightly lower than in run 46, and CVs around these estimates were slighlty higher at the end of the time series. 

AIC was smaller for run 46A but had larger Mohn's rho values than run 46. Since there were no major improvements to fit to fleet or index data, there is a slight preference for run 46.
