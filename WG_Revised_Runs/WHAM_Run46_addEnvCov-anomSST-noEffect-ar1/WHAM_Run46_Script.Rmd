---
title: "WHAM_Run46_addEnvCov-anomSST-noEffect-ar1"
output: html_document
date: '2022-05-17'
---

## Model description
Model specified as in run 39 (fit with an environmental covariate that has no effect so it can be directly compared to run 46A that incorporates this environmental effect on recruitment) but implements an ar1 process rather than a random walk. Annual sea surface temperature anomaly was the environmental covariate considered here, with a 1 year lag (bottom temperature in year t impacts recruitment in year t+1) and a random walk process model.
...
input <- prepare_wham_input(asap3, NAA_re = NAA_re, selectivity = sel_list, ecov=ecov, model_name = "WHAM_Run46") 
```

### Save input
```{r}
# Save model data input
saveRDS(input, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run46_addEnvCov-anomSST-noEffect-ar1", "WHAM_Run46_input.rds", sep="/"))
```

### Fit model, check convergence, and run diagnostics
```{r}
WHAM_Run46 <- fit_wham(input, do.osa = TRUE, do.retro = TRUE) 
check_convergence(WHAM_Run46)
print(paste("Number of parameters", length(WHAM_Run46$par), sep=" "))

plot_wham_output(mod=WHAM_Run46, out.type='html')
# plot_wham_output(mod=WHAM_Run46, out.type='png', dir.main = paste(here::here(), "WHAM_Run46", sep="/"))
```

### Save output
```{r}
# Save fitted model
saveRDS(WHAM_Run46, file=paste(here::here(), "WG_Revised_Runs", "WHAM_Run46_addEnvCov-anomSST-noEffect-ar1", "WHAM_Run46_model.rds", sep="/"))
```
...
inputRerun <- readRDS(paste(here::here(), "WG_Revised_Runs",
                     "WHAM_Run46_addEnvCov-anomSST-noEffect-ar1/WHAM_Run46_input.rds", sep="/"))

# Rerun data
ReRun46 <- fit_wham(input = inputRerun, MakeADFun.silent = TRUE)
```

## Comment
AIC was smaller for run 46A (see comments in that run script), but had larger Mohn's rho values than run 46. Since there were no major improvements to fit to fleet or index data, there is a slight preference for run 46.

Note: Not all results plots generated for run 46