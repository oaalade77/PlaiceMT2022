...
# Fit plaice model
plaice_model <- fit_wham(input) # https://timjmiller.github.io/wham/articles/ex7_debug.html # do.fit = F returns an error
...
# Fit plaice model
plaice_model1 <- fit_wham(input1) # https://timjmiller.github.io/wham/articles/ex7_debug.html # do.fit = F returns an error
check_convergence(plaice_model)
...
# Fit plaice model
plaice_model2 <- fit_wham(input2) # https://timjmiller.github.io/wham/articles/ex7_debug.html # do.fit = F returns an error
...
fix_index_sel = lapply(1:input$data$n_indices, function(x) 9) # Index NA means do not fix them
fix_fleet_sel = lapply(1:input$data$n_fleets, function(x) NA) #need this for fleets
...
###### Fit plaice model
plaice_model3 <- fit_wham(input3, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model4 <- fit_wham(input4, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model5 <- fit_wham(input5, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model6 <- fit_wham(input6, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model7 <- fit_wham(input7, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model8 <- fit_wham(input8, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model9 <- fit_wham(input9, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model10 <- fit_wham(input10, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
###### Fit plaice model
plaice_model11 <- fit_wham(input11, do.osa = FALSE,do.retro=FALSE,do.check=TRUE) #do.check will suggest where to look for and parameters that are challenging to estimate in current config
...
# Notes on data
I had a productive hour not paying attention to my boring call and playing with plaice data in ASAP. 1) There are some errors in the .dat you sent me. I fixed some data errors, but I'm not sure if my solutions were appropriate. Easiest thing would be to set up another chat and I can share my screen and discuss. 2) Some of the structural settings were nonsensical, typically as it pertains to selectivity. I fixed those, but again, worth a chat. 3) The attached dat "works" in that it converges and runs in ASAP. The model fit is ugly and I did not spend any time refining this model. 4) The attached dat will convert to WHAM and run. Although I did get an error message about the covariance matrix, I suspect due to some crazy high parameter correlations. It should at least give you a starting place to horse around in WHAM, however. 5) I have some ideas about changes to make in ASAP that would likely fix some problems and also result in a better WHAM transition. Again, for discussion later. - from JD
